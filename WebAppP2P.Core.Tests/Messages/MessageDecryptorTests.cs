...
namespace WepAppP2P.Core.Tests.Messages
{
    public class MessageDecryptorTests
    {
        ...
        [InlineData(true)]
        [InlineData(false)]
        public void MessageDecryptor_Decrypt_Should_Decrypt_When_Has_Valid_Data(bool privateKeyIsReciever)
        {
            var title = "tit";
            ...
            var msgd = md.Decrypt(msg, privateKeyIsReciever ? toPrivateKeyBase64 : Convert.ToBase64String(keyPairTest.PrivateKey), privateKeyIsReciever);

            Assert.True(msg.Id == msgd.Id);
            Assert.True(msg.Timestamp == msgd.Timestamp);
            Assert.True(msg.To == msgd.To);
            Assert.True(msg.From == msgd.From);
            Assert.True(title == msgd.Title);
            Assert.True(content == msgd.Content);
        }
